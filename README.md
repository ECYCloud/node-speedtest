...
| **Linux** | x86_64 | `.deb` / `.rpm` / `.AppImage` | ⚠️ 实验性,见下面平台说明 |
| **Linux** | arm64 | `.deb` / `.rpm` / `.AppImage` | ⚠️ 实验性 |
...
| RHEL / CentOS Stream 9, AlmaLinux / Rocky Linux 9, openSUSE Leap 15.6+ | `.rpm` | 仓库提供 `webkit2gtk4.1`,rpm 直接 `dnf install` 即可 |
| **Ubuntu 25.04+, Fedora 40+, Arch** | **`.AppImage`** | 这些发行版主仓库已移除 webkit2gtk-4.1(切到 6.0,Tauri 2 暂未适配),`.deb` / `.rpm` 装上也起不来;AppImage 自带 webkit runtime |
| 其他 | 优先 AppImage | 自包含,无需考虑发行版差异 |

AppImage 用法:`chmod +x Stair.Speedtest_*.AppImage && ./Stair.Speedtest_*.AppImage`

RPM 用法:`sudo dnf install ./stair-speedtest-*.rpm`(老系统用 `sudo rpm -i ./stair-speedtest-*.rpm`)。
